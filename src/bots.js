...
export class BotManager{
  constructor(scene){
    this.scene = scene;
    this.loader = new GLTFLoader(); this.fbx = new FBXLoader();
    this.bots = [];
  }
  ...
  addGLTFCharacter(url, options={}){
    ...
    const onAdd = (obj)=>{
      ...
      root.traverse(o=>{ o.userData.hittable = true; });
      root.userData.damageable = true;
      root.userData.health = options.health ?? 100;
      this.scene.add(root);
      this.bots.push({ object:root, state:'idle' });
    };
    if(ext==='fbx'){
      this.fbx.load(url, onAdd);
    }else{
      this.loader.load(url, onAdd);
    }
