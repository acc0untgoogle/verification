...
tryLoadMap(loader, scene, 'assets/map.glb').catch(()=>{
  createBasicMap(scene);
});

// DEBUG fallback enemies
bots.spawnRandom(6);
...
function loop(){
  ...
  weapon.update(dt, player.getDirection(), (origin, dir)=>{
    ...
    if(hit){
      if(hit.object.userData?.damageable){
        bots.applyDamage(hit.object, weapon.damage);
      }
      // optional: small spark
...
function hitscan(origin, dir, dist=200){
  ...
  const candidates = [];
  scene.traverse(o=>{
    if(o.userData?.hittable) candidates.push(o);
  });
...
// Export hooks for asset swapping
